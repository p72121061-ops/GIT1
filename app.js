// ─── SUPABASE CONNECTION (global, shared by all scripts) ───
...
(function(){
  ...
  async function syncToSupabase(){
    try{
      const rows = students.map(s=>({id:s.id,name:s.name,cls:s.cls,phone:s.phone,roll:s.roll,color:s.color,code:s.code,fees:s.fees}));
      const { error } = await sb.from('students').upsert(rows);
      if(error) console.warn('Supabase sync failed (data still saved locally):', error.message);
    }catch(e){ console.warn('Supabase sync error (data still saved locally):', e); }
  }

  // Pull latest data from Supabase (other devices' changes) and refresh UI
  async function loadFromSupabase(){
    try{
      const { data, error } = await sb.from('students').select('*').order('id');
      if(error){ console.warn('Supabase load failed, using local data:', error.message); return; }
      if(data && data.length){
        students = data.map(r=>({id:r.id,name:r.name,cls:r.cls,phone:r.phone,roll:r.roll,color:r.color,code:r.code,fees:r.fees||{}}));
        localStorage.setItem(STORE,JSON.stringify(students));
        faRender();
      } else if(students && students.length){
        // Supabase empty but local has data (first migration) — push it up
        syncToSupabase();
      }
  ...
  function cacheNotices(arr){ localStorage.setItem(NOTICE_KEY, JSON.stringify(arr)); }

  async function loadNoticesFromSupabase(){
    try{
      const { data, error } = await sb.from('notices').select('*').order('id',{ascending:false});
      if(error){ console.warn('Notices load failed, using local cache:', error.message); return; }
      if(data){
        cacheNotices(data);
        renderPublicNotices();
        if(document.getElementById('noticeAdminList')) renderNoticeAdmin();
      }
    }catch(e){ console.warn('Notices load error:', e); }
  }
  window.loadNoticesFromSupabase = loadNoticesFromSupabase;
  ...
  window.addNotice = async function(){
    ...
    const notice = { id: Date.now(), tag, title, body, date: new Date().toLocaleDateString('en-IN',{month:'short',year:'numeric'}) };
    ...
    renderNoticeAdmin();
    const { error } = await sb.from('notices').insert(notice);
    if(error) console.warn('Notice sync to Supabase failed (saved locally):', error.message);
  };

  window.deleteNotice = async function(id){
    if(!confirm('Delete this notice?')) return;
    cacheNotices(getNotices().filter(n=>n.id!==id));
    renderNoticeAdmin();
    renderPublicNotices();
    const { error } = await sb.from('notices').delete().eq('id', id);
    if(error) console.warn('Notice delete sync failed:', error.message);
  };
  ...
  window.saveUpiSettings = async function(){
    ...
    const rows = [
      {key:'vt_upi_id', value: upiId},
      {key:'vt_upi_name', value: payeeName || 'Vrindavan Tutorials'},
    ];
    if(teacherPhone) rows.push({key:'vt_teacher_phone', value: teacherPhone});
    const { error } = await sb.from('settings').upsert(rows);
    if(error){
      console.warn('Settings sync to Supabase failed (saved locally):', error.message);
      alert('✅ Saved locally. ⚠️ Cloud sync failed — students on other devices may not see this update yet.');
    } else {
      alert('✅ Settings saved! All future payments and alerts will use this info.');
    }
  };

  // Pull latest settings from Supabase so every visitor (any device) sees the current UPI info
  async function loadSettingsFromSupabase(){
    try{
      const { data, error } = await sb.from('settings').select('*');
      if(error){ console.warn('Settings load failed, using local cache:', error.message); return; }
      if(data && data.length){
    ...
    const toppersListHtml = toppers.length ? toppers.map(t=>`
      <div style="background:#FFFFFF;border:1px solid #DDD2BC;border-radius:10px;padding:10px 14px;margin-bottom:8px;display:flex;align-items:flex-start;gap:10px;">
        <div style="flex:1;">
          ...
          <div style="font-size:11px;color:#DDD2BC;margin-top:3px;">💬 @${t.whatsapp}</div>
        </div>
  ...
  window.faOpenAddTopperModal = function(){
    ...
    document.getElementById('atWhatsapp').value = '';
  ...
  window.faSaveTopper = function(){
    ...
    const whatsapp = document.getElementById('atWhatsapp').value.trim().replace(/^@/,'').toLowerCase();
    const achievement = document.getElementById('atAchievement').value.trim();
    if(!name || !whatsapp){ alert('Name and WhatsApp username are required!'); return; }
    const maxId = toppers.length ? Math.max(...toppers.map(t=>t.id)) : 0;
    toppers.push({id: maxId+1, name, whatsapp, achievement: achievement || 'Topper at Vrindavan Tutorials'});
    localStorage.setItem(TOPPER_STORE, JSON.stringify(toppers));
  ...
  faRender();
  loadFromSupabase(); // background sync — pulls in any updates from other devices
})();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if(typeof loadNoticesFromSupabase === 'function') loadNoticesFromSupabase();
    if(typeof loadSettingsFromSupabase === 'function') loadSettingsFromSupabase();
    ...
    function showAssignedTopper(topper, code){
        ...
        const waLink = `https://wa.me/${topper.whatsapp}?text=${encodeURIComponent('Hi '+topper.name+', I got your contact from Vrindavan Tutorials — I had a few questions!')}`;
        document.getElementById('tpBody').innerHTML = `
            ...
            <a class="tp-submit-btn" href="${waLink}" target="_blank" style="display:block;text-align:center;text-decoration:none;">💬 Chat with ${topper.name} on WhatsApp</a>
            <div class="tp-note">🔒 This opens a WhatsApp chat using their username — their phone number is never shown to you. This code has now been used.</div>
        `;
